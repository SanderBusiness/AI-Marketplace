...
| `NEW_RELIC_REGION` | `eu` or `us`, the data centre of the account. EU accounts use `api.eu.newrelic.com`. Defaults to `eu` in the script. | No |
...
If `NEW_RELIC_API_KEY` is not set, `scripts/nrql.sh` reads the key from the macOS Keychain
...
- Never write it into a file that is committed (`.env`, scripts, settings, notes) or into
  persistent memory.
- The script passes the key to `curl` through stdin, so it does not appear in the process
  list. Keep it that way if you change the script.
